...
bool EXIT;
void pre_auton(void)
{ // prepare for auto, by setting default pneu states and zeroing gyro
  // ATaskActiv = 1;
  EXIT = false;
  Clamp.set(false);
  Doinker.set(false);
  ...
  {
    // Set degrees to 359, due to macro issues
    //  (runs in wrong way if zeroed to 0 or above.)
...
int ITask(void) {
    ...
    while (true) {
        ...
        while (Eject == 1) {
          ...
          In1.setBrake(brake);
          olddegree = abs(In1.position(degrees)) - 70;
...
int XTask(void)
{
  while (true)
  {
    ...
    if (XTaskActiv == 0 && Controller1.ButtonX.pressing() && ButtonPressingX == 0) // Finding if ButtonX is pressing and if it was held down before.
    {
      ...
      ButtonPressingX = 0; // Checks if the botton isn't being held down
...
int YTask(void)
{
  while (true)
  {
    ...
    if (YTaskActiv == 0 && Controller1.ButtonY.pressing() && ButtonPressingY == 0) // Finding if ButtonY is pressing and if it was held down before.
    {
      ...
      ButtonPressingY = 0; // Checks if the botton isn't being held down
...
int BTask(void)
{
  while (true)
  {
    ...
    if (BTaskActiv == 0 && Controller1.ButtonB.pressing() && ButtonPressingB == 0) // Finding if ButtonY is pressing and if it was held down before.
    {
      ...
      ButtonPressingB = 0; // Checks if the botton isn't being held down
